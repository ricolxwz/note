...
2. 如果模型在安静的环境下识别准确, 但是在嘈杂环境在下降明显, 怎么改进?
    ...
    2. 前端方面: 单独先做一个"通用降噪", 再把增强后的波形喂给ASR, 未必会更好; 原因是增强会引入语音失真, ASR不一定喜欢; 更稳的是: **一条是前端增强和ASR联合训练, 另一条是把noisy分支和enhanced分支做融合, 而不是只保留增强后的音频**. 近期有研究发现, 零样本场景下, 通用降噪即使让听感更干净, 也可能让Whisper等模型识别更差.

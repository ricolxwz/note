...
智能指针是用来自动管理内存生命周期的对象, 定义在<memory>里面, 核心思想是RAII, 资源在对象构造的时候获取, 对对象析构的时候自动释放, 避免手动new/delete导致的内存泄露.

- std::unique_ptr: 独占所有权, 不能拷贝, 只能移动
- std::shared_ptr: 共享所有权, 引用计数为0的时候释放资源
- std::weak_ptr: 弱引用, 不增加引用技术, 常用于解决循环引用, 如果两个对象互相持有`std::shared_ptr`, 就可能出现循环引用, 导致引用计数永远不为0, 内存无法释放

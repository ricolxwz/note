...
2022年以来主流系统越来越多采用"强预训练单模态模型 + 轻量跨模态桥接模块 + 生成式语言接口"的架构: 视觉侧用强视觉编码器, 语言侧用大语言模型, 中间用适配器/查询Transformer/交叉注意力把视觉信息注入语言生成, 从而把多模态能力统一为"对话式问答/解释/推理". 

优势在于: 数据效率与训练效率: 冻结大部分参数, 只训练桥接模块或少量层, 显著降低训练成本; 统一交互接口: 自然语言成为多任务"控制面板"(指令即任务), 使系统更贴近产品形态; 能力组合: 可把检索, 工具, OCR, 结构化输出等纳入统一解码框架. 局限在于幻觉与跨模态一致性: 生成式接口更容易产生"看似合理但与视觉不一致"的输出, 需要专门评测与训练约束; 长上下文与多图/视频: token爆炸带来推理成本与延迟压力; 数据治理与安全: 训练语料的版权, 隐私与有害内容会反映到模型输出能力边界, 需要系统化过滤与评估. 

...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/43f2112e139a0a3fc85bfd86b6e60771.webp#only-light){ loading=lazy width='500' }
  ![](https://img.ricolxwz.download/43f2112e139a0a3fc85bfd86b6e60771_inverted.webp#only-dark){ loading=lazy width='500' }
...
对于模型的overview如上图所示. 这个标准的Transformer会收到1D的嵌入序列. 为了处理2D的图像, 他们将图像$\mathbf{x} \in \mathbb{R}^{H \times W \times C}$首先转化为一个展平的2D patches $\mathbf{x}_p \in \mathbb{R}^{N \times (P^2 \cdot C)}$, $(H, W)$表示的是原图的分辨率, $C$表示的是通道数, $(P, P)$表示的是patch的分辨率, $N=HW/P^2$表示的是产生的patches的数量, 同时也是输入Transformer的序列长度. Transformer对于所有的层潜向量大小是固定的$D$, 所以他们使用可训练的线性变换对这些patches进一步展平到$D$维, 他们将这个线性变换的结果叫做patch嵌入(如下面的第一个公式所示).
...
???+ note "为什么MLP被认为是Local的"
    ...
    对于MLP, local不是指图像空间上的locality, 是指MLP对每个输入的patch embedding独立, 相同的进行处理(共享权重), 并不考虑其他patch的信息(因为它没有像自注意力那样的全局关联计算), 换句话说, 对于序列中的每个embedding, MLP执行的变换是相同的. 从另一个角度理解, MLP是point-wise的操作, 它不会根据全局结构或者位置差异对不同的embedding赋予不同的处理方式, 而是对每个embedding单独且同样地变换.

    对于MSA, 自注意力机制会计算序列中任意两个embedding之间的注意力权重, 也就是说, 输出不仅仅取决于每个embedding自身的特征, 还高度依赖于其他embedding的关系, 能够“看到”整个序列的全局结构和上下文信息, 因此具有非局部性.

???+ note "为什么MLP被认为是Translationally Equivariant的"

    “平移不变形”指的是, **~~无论特征出现在图像的哪个位置, 网络都能在那个相应的位置产出类似的响应, 并且模型无需为不同位置的同类特征单独学习不同的参数~~, 无论输入embedding的位置如何变化, 对每个embedding的处理方式都相同.**

    对于MSA, 进行平移后, 不同embedding可能相对位置会发生变化, 导致对于每个embedding来说, 最终的注意力得分发生变化. 所以对每个embedding来说, 其注意力处理方式是会发生变化的. 对于MLP, 不管如何进行平移, 对于每个embedding的处理方式都是不会发生变化的, 都是经过相同的非线性变换.
...
通常, 我们现在大数据集上预训练ViT, 然后微调以用于(较小的)下游任务. 在进行下游任务微调的时候, 由于下游任务的类别数量$k$可能和预训练的时候不同, 此时需要移除预训练时候的分类头, 或者说是预测头, 并添加一个新的维度为$D\times k$的线性层作为新的预测头.

此外, 预训练通常在标准的分辨率下进行(如224\*224), 此时, 模型学习到了一定的视觉和模式, 当在fine-tuning时使用高分辨率的图像(如384\*384)的时候, 图像中会出现更多的细节特征(如更清晰的纹理, 边缘, 微小物体或者局部结构). 由于patch的大小保持不变(仍为16\*16), 更高分辨率的图像会被分割出更多的patch, 从而序列长度增大. 这意味着在微调的时候能够获得更加密集, 更加细粒的输入, 从而提升对下游任务的识别, 分类或者检索性能. 理论上来说, 只要有足够的内存, ViT就能处理任意长度的序列, 但是有一个问题, 在预训练的时候使用的是对应于224\*224图片的patch网格的positional embeddings. 如果patch网格产生变化, 那么原来的positional embeddings就完全不能用了, 因此预训练的模型就失去了意义. 这可以通过2D插值来实现. 当我们进行这种2D插值的时候, 其实是引入了patch网格的二维空间信息, 因为插入的位置编码是基于它在空间中的位置的, 除了patch内部的2D结构之外还有这个之外, 所有的2D结构都要通过模型自己学习.

???+ note "什么是插值"

    将预训练好的位置嵌入向量阵列视为一个14\*14的特征图, 然后通过类似图像插值的方法将其扩展到24\*24的特征图, 以匹配新分辨率下的网格. 拿一个简单的1D插值来说, 比如原来的位置编码是$7, 9$, 那么经过插值之后, 原来的位置编码是保持不变的, 再中间插值, 变成了$7, 8, 9$. 只不过, 它这里用是2D的插值, 需要对二维网格上的相邻位置编码做平均.

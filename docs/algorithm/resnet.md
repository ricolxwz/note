...
<figure markdown='1'>
![](https://img.ricolxwz.download/bca8ca0cfe726369f49294dd19322075.png){ loading=lazy width='600' }
...
<figure markdown='1'>
![](https://img.ricolxwz.download/2c69b6152dba08a21c64cb133c5020a2.png){ loading=lazy width='600' }
...
<figure markdown='1'>
![](https://img.ricolxwz.download/0ac52a866f3a39e361c7406ff4e8a214.png){ loading=lazy width='400' }
</figure>
...
<figure markdown='1'>
![](https://img.ricolxwz.download/5637f76757e67bc2888327a110a9937a.png){ loading=lazy width='350' }
...
2. 使用1*1的卷积核进行升维操作. 具体操作参考[这里](/algorithm/cnn/#increase-dimension)

## 实现

作者对于ImageNet上训练的ResNet实现如下:

在训练的时候, 将图片的最短边随机resize到[256, 480], 然后参考AlexNet中的[随机裁剪](/algorithm/alexnet/#random-cropping)对图片进行224*224的随机裁剪(包括它的水平反转)以防止过拟合. 然后, 减去每个像素的平均值, 去掉很多不必要的背景信息, 例如全局亮度 :fontawesome-solid-circle-question:. 然后在每一个卷积层之后采用BN. 对所有的参数进行从头训练. 采用mini-batch SGD, batch的大小为256. 学习率从0.1开始下降当错误率遇到平稳的情况除以10避免震荡. 采用0.0001的权重衰减(就是[L2正则化](/algorithm/linear-regression/#l2-regularization), 防止模型过拟合), [动量](/algorithm/fnn/#动量)设置为0.9. 并没有使用Dropout.

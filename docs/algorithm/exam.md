...
- [神经网络](/algorithm)
    ...
    - ☢️[前向传播](/algorithm/fnn/#backpropagation-algorithm), 给出一张网络的[图](http://img.ricolxwz.asia/58a62f5af6cb3f0dcd287eb696e918a8.png), 计算最后的输出. 使用的是sigmoid函数, $y=1/(1+e^{-x})$, 这个函数记住
    ...
    - ⚠️[神经元的数量](/algorithm/fnn/#neuron-num). 从较小的网络开始, 慢慢训练较大的网络, 直到准度不再升高
    - ⚠️[学习率大小](/algorithm/fnn/#learning-rate). 学习率太小, 收敛很慢, 学习率太大, 可能造成震荡, 正确的做法是随着训练轮次的增加, 减少学习率
    - ♻️[Dropout](/algorithm/#dropout). 每次反向迭代的时候, 随机选择部分神经元, 将其输出设置为$0$表示丢弃
    - ♻️[动量](/algorithm/fnn/#动量), 减少震荡的发生, 增大学习率, 方法是引入之前梯度更新的累积量, $\Delta w_{pq}=\eta\cdot \delta_q\cdot o_p+\mu (w_{pq}(t)-w_{pq}(t-1))$
    - ♻️[权重初始化策略](/algorithm/fnn/#weight-initialization). 在-1->1内随机初始化或者从正态分布中随机采样, 标准差是$\sigma=\sqrt{\frac{2}{N_{in}+N_{out}}}$, 其中$N_{in}$是输入神经元的数量, $N_{out}$是输出神经元的数量, 注意, 这里的输入输出神经元不是整个神经网络的输入输出神经元, 是相对于当前层神经元来说的上一层/下一层神经元, 当前层神经元就是权重/截距待更新的神经元

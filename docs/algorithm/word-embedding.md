...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/c2ca6351fdf76fe5b747211d159b8472.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/c679a2cc7a679aaa676627f906fc80b4.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/0455d8f6163ce2dc94ea360c32a9012d.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/4927908c6964b1d8d55d9008a7e543ac.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/d89785032c4d1b77f250e06a473e0d70.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/bedd9aba9a35a07b6bb3d6d499077f07.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/d999f07762bbe97c0eb0c884eb2220f3.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/fc0d12a75b44f0ad50c7817db6b35c6d.png){ loading=lazy width='700' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/1d1a0c6e49eaa13069990aa098f06e5c.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/a0fdd6fae7201e9347ff9a8a2199313c.png){ loading=lazy width='500' }
</figure>

实际上更加常见的做法是使用一个固定的历史窗口, 举个例子, 你总是想预测四个单词后的下一个单词, 注意这里的4是超参数. 这就意味着你的神经网络输入的不是一个6个300维的向量, 而是4个300维的向量.

<figure markdown='1'>
  ![](https://img.ricolxwz.download/c20828ff376bed49b8ce6268ffad46c0.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/623d942029f748dededcff06919d5587.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/4ad603bef5dddd0461c51d8145db5ae4.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/8378cbeae5c72435e821c08ce1a3a0c7.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/251511adf4332b5ca4ae05c4adbe8dc1.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/1128fac230e745b4f267079c02ff6cf1.png){ loading=lazy width='500' }
</figure>

???+ note "优化目标和任务的区别"

    请见[这里](/algorithm/gpt/#intro).

接下来, 我们要构造一个稍微不一样的优化目标, 其中输入为$x$(上图编号为7), 要去预测标签$y$(上图编号为8). 这个算法就是要分辨这两种不同的采样方式. 即原来的优化目标1是给定上下文词, 预测目标词; 优化目标2(现在的新优化目标)是给定上下文词和目标词, 判断是正采样还是负采样, 这就避免了对整个词汇表进行softmax预测, 将原本的预测目标词的问题转化为一个二分类问题的形式. 也就是说, 采用了负采样的skip-gram模型, 会直接采用优化目标2的形式, 而不会执行优化目标1.

如何选取$K$? 原文作者推荐如果小数据集的话, $K$从5到20比较好, 如果数据集比较大, $K$就小一点, 对于更大的数据集$K$就等于2到5.

<figure markdown='1'>
  ![](https://img.ricolxwz.download/1fe9baf0e00324f77d80153af58395b1.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/a16f699705c14f2d8950b92c9745694d.png){ loading=lazy width='500' }
</figure>
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/c0f1d855a804b0c9160c7347e28a9bac.jpg){ loading=lazy width='200' }
  <figcaption>Skip-Gram模型</figcaption>
</figure>

<figure markdown='1'>
  ![](https://img.ricolxwz.download/b64e1c1dbc76bc2c893d6ac0c98ff2f4.jpg){ loading=lazy width='200' }
...
在之前, 我们曾通过挑选语料库中位置相近的两个词, 列举出词对, 即上下文和目标词. GloVe算法所做的就是使其关系明确化. 假定$X_{ij}$是单词i在单词j上下文中出现的次数, 这里的$i$, $j$就和$t$, $c$的功能一样. 注意, 当我们将上下文和目标词的范围定义为出现在左右10个词以内的时候, $X_{ij}$和$X_{ji}$是对称的, 即$X_{ij}=X_{ji}$. 这是因为如果词$i$出现在$j$的上下文中, 那么反过来$j$也会出现在$i$的上下文中. 但是如果我们将上下文定义为只出现在目标词一侧的话, 那么这种对称性就消失了. 在GloVe中, 我们采用对称的上下文, 即$X_{ij}=X_{ji}$, 这些计数器构成了共现矩阵, 即这个矩阵里面的元素$X_{ij}$表示的是单词$i$和单词$j$出现位置相近或是彼此接近的频率的计数器.

🌟在GloVe模型中, 这个共现矩阵是已知的. 在模型训练之前, 我们会从一个非常庞大的语料库中统计词与词之间的共现关系, 这个过程会产生一个矩阵, 也就是共现矩阵$X$. GloVe的优化目标是使得词向量(或者说是嵌入矩阵)能够很好的体现这个共现矩阵.🌟 通过公式可以描述为:
...
<figure markdown='1'>
  ![](https://img.ricolxwz.download/4e3c06ef5d59815da01c40a227f227bc.png){ loading=lazy width='500' }
</figure>

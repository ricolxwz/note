...
Transformer 在语音领域的研究, 主要围绕这5个方向展开:

* 鲁棒性提升(稀疏输入, 对抗攻击)
* 特征增强(多流, 多层融合)
* 效率优化(轻量化, 自监督)
* 结构创新(dual-path, memory attention)
* 应用扩展(ASR, 分离, 增强, 音频描述)

---

* 鲁棒性提升:
    * 输入稀疏性: 研究发现, Transformer比CNN更加容易受到输入稀疏(比如静音段)的影响. 这是因为Transformer依赖注意力机制, 输入信息稀疏的时候, 注意力容易失效或者分散. 研究提出一种正则化的方法, 惩罚attention分配到静音帧上的权重, 从而引导模型更加关注有效语音区域.
    * 对抗攻击: 方案一: Vision Transformer + 滤波器增强模块  (low-pass/band-pass), 通过过滤掉高频噪声干扰, 增强模型抵抗力; 方案二: 用Transformer检测哪些音频是被攻击过的, 准确率高.

        ??? tip "什么是对抗攻击"

            攻击者往正常音频里面添加人耳几乎听不出来的噪音, 让AI模型误判. 例子:

            - 正常语音: "播放音乐" → AI听成 ✅
            - 被攻击后: "播放音乐" + 微量噪音 → AI听成 ❌ "暂停"

* 特征增强:
    * 中间特征多样性不够: 第一个方案是构建多层声学特征提取框架, 浅层是传统特征(利于分类), 浅层是传统特征(利于分类), 深层是更加抽象, 与说话人无关的特征. 第二个方案是Feature Correlation Fusion(FCF), 时域特征和频域特征各有优劣, 通过相关性融合让两者互补, 提取更加全面的表示.
* 效率优化:
    * 自监督学习MAE-AST: MAE-AST的设计灵感来源于BERT. 但是只用未mask的token做编码. Encoder负责处理未遮蔽的token, Decoder负责重建被遮蔽的部分, 下游任务只使用训练好的Encoder. 优势是训练更快, 内存更省, 性能超过SSAST.

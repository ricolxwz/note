...
| 方案名称       | 任务类别     | 基础类型         | 驱动类型   | 核心技术                                                         |核心优点|核心缺点|
|---------------|--------------|------------------|------------|------------------------------------------------------------------|--|--|
| PCH           | 听者头部生成 | 非LLM基础        | 音频驱动   | 增强版神经渲染器           |提升画面的质量和稳定性|专注于渲染而不是动作生成|
| RLHG          | 听者头部生成 | 非LLM基础        | 多模态驱动 | 基于LSTM的seq-to-seq模型         |完整的baseline|不具有多样性|
| L2L           | 听者头部生成 | 非LLM基础        | 多模态驱动 | VQ-VAE离散编码                                     |产生多样的输出|对特定的动作不具有较好的控制性|
| ELP           | 听者头部生成 | 非LLM基础        | 多模态驱动 | VQ-VAE高维离散编码, 根据情感先验重排潜空间  |能够控制听者的态度, 产生符合情感的结果||
| MFR-NET       | 听者头部生成 | 非LLM基础        | 多模态驱动 | 扩散模型, 特征聚合保持身份一致               |产生多样化的动作, 身份保持||
| SaRLHS        | 听者头部生成 | 非LLM基础        | 多模态驱动 | 文本语义特征, 听者头部生成网络               |||
| ViCo          | 听者头部生成 | 非LLM基础        | 多模态驱动 | 对话视频基准数据集; 基线采用3DMM系数驱动 + 神经渲染管线           |||
| DIM           | 听者头部生成 | 非LLM基础        | 多模态驱动 | 掩码对比预训练(Transformer)+ VQ-VAE离散运动表示               |||
| RealTalk      | 听者头部生成 | 基于LLM (检索式) | 文本驱动   | 大规模真实双人对话视频数据集(含转录文本)                       |||
| CLMLtL        | 听者头部生成 | 基于LLM (生成式) | 文本驱动   | VQ-VAE动作离散化 + 自回归语言模型(GPT类Transformer)            |||
| CustomListener| 听者头部生成 | 基于LLM (生成式) | 多模态驱动 | Diffusion扩散生成架构 + Transformer解码器控制                    |||
| Wav2Lip       | 说者头部生成 | 非LLM基础        | 音频驱动   | GAN对抗生成(CNN声码器+判别器), 音频驱动唇动同步                |||
| Face2Face     | 说者头部生成 | 非LLM基础        | 视频驱动   | 3D人脸Morphable模型拟合 + 图形学纹理重映射实时再现               |||
| FOMM          | 说者头部生成 | 非LLM基础        | 视频驱动   | 关键点驱动稠密光流变形 + CNN图像生成器                           |||
| AgentAvatar   | 对话头部生成 | 基于LLM          | 多模态驱动 | 大型语言模型计划生成 (LLM) + 动作序列编码 (token) + NeRF 渲染器  |||
| INFP          | 对话头部生成 | 非LLM基础        | 音频驱动   | 双阶段框架: VAE运动潜码表示 + 扩散模型音频到动作生成             |||
| ViCo-X        | 对话头部生成 | 非LLM基础        | 多模态驱动 | 分角色双模型(说话人&聆听者)+ 角色切换控制机制                  |||
| DialogueNeRF  | 对话头部生成 | 非LLM基础        | 多模态驱动 | 动态NeRF神经辐射场 + 音频驱动说话人 & 音频/视觉驱动聆听者         |||
| AvatarGPT     | 对话头部生成 | 基于LLM          | 多模态驱动 | 大型语言模型(GPT)+ 离散动作token扩展词表 + 多任务训练          |||
| MotionCLIP    | 整体运动生成 | 非LLM基础        | 文本驱动   | Transformer骨架运动AutoEncoder + CLIP跨模态对齐                   |||
| TEMOS         | 整体运动生成 | 非LLM基础        | 文本驱动   | 文本条件VAE(变分自编码)+ Transformer运动解码                   |||
| GDN2HMT       | 整体运动生成 | 非LLM基础        | 文本驱动   | 扩散模型 + 动作风格迁移引导(少样本两阶段迁移)                   |||
| MDM           | 整体运动生成 | 非LLM基础        | 文本驱动   | 扩散模型(无分类器引导)+ Transformer时序U-Net架构               |||

...
tensor参数可以自动扩展为相同的大小. 广播机制需要满足两个条件: 每个张量至少有一个维度, 满足右对齐. 什么是右对齐, 例如`torch.rand(2, 1, 1)+torch.rand(3)`, 右边的可以看作是`torch.rand(1, 1, 3)`, 满足右对齐的条件是, 从右到左, 每个维度上要么是相等的, 要么至少有一个`1`, 比如, 最右边是`1`和`3`, 有一个`1`, 中间是`1`和`1`, 有两个`1`, 左边是`2`和`1`, 有一个`1`, 因此是右对齐的. 更多细节可以参考NumPy的广播机制: https://note.ricolxwz.de/numpy/broadcast/.
...
???+ note "PCA和LDA的区别图例"
    ...
    <figure markdown='1'>
    ![](https://img.ricolxwz.io/0b2064409573b415daa687b04e429fcf.webp#only-light){ loading=lazy width='800' }
    ![](https://img.ricolxwz.io/0b2064409573b415daa687b04e429fcf_inverted.webp#only-dark){ loading=lazy width='800' }

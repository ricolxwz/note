...
2.  关键字: 使用`inline`关键字向编译器建议将函数内联.
    ```cpp
    inline int min(int a, int b) {
        return (a < b) ? a : b;
    }
    ```
3.  编译器决策: `inline`只是一个请求, 并非强制命令. 编译器会根据多种因素 (如函数体积, 优化级别, 是否递归等) 自行决定是否真正内联一个函数.
4.  定义位置: 内联函数的定义通常放在头文件中. 因为编译器在编译调用点时需要知道函数的完整定义才能进行内联展开. 如果定义放在`.cpp`文件中, 其他编译单元可能无法内联它.
5.  适用场景: 通常适用于函数体小, 调用频繁的函数.
6.  潜在影响:

...
???+ info "信息"
    ...
    - 前置知识
        - [结构化数组](/language/python/numpy/structured-array): [设置名称](#设置名称), [调整转换](#调整转换), [快捷函数](#快捷function).

根据文件的不同, 可以将用于IO的文件分为二进制文件和文本文件, 关于它们的区别, 见[这里](/language/python/foundation/编码#文本文件和二进制io).

## 二进制文件

在[数组构建](/language/python/numpy/array-creation)章节中, 我们介绍了简单的读写HDF5二进制文件的[方式](/language/python/numpy/array-creation/#读取二进制io). 
...
- `[arr]`: 要保存的数组
- `allowpickle=[True/False]`: `pickle`模块的作用见[这里](/language/python/foundation/io/#序列化对象为二进制io). 若为`True`, 则允许保存含有对象的数组, `pickle`模块会在保存和读取之前, 对数组里面的对象进行序列化和反序列化
...
- `[*args]`: 要保存的数组, 使用[可变参数](/language/python/foundation/function/#可变参数)传入, 数组会自动起名为`arr_0, arr_1, ...`

- `[**kwds]`: 要保存的数组, 使用[关键字参数](/language/python/foundation/function/#关键字参数)传入

## 文本文件

在[数组构建](/language/python/numpy/array-creation)章节中, 我们介绍了简单的读写csv文本文件的[方式](/language/python/numpy/array-creation/#读取文本io). 
...
`np.genfromtxt()`唯一的强制参数是数据源. 它可以是字符串, 字符串列表, 返回字符串的生成器或具有`read`方法的打开的文件类对象, 例如文件或`io.StringIO`对象, 详情见[这里](/language/python/foundation/io/#StringIO和BytesIO). 
    ...
    ???+ tip "Tip"
        ...
        - 默认情况下, `delimiter`默认值为`None`. 这意味着每一行按照空白进行拆分, 连续的空白被视为单个空白.
            ...
            ???+ example "例子"
                ...
                File ~/miniconda3/envs/test/lib/python3.12/site-packages/language/python/numpy/lib/npyio.py:2312, in genfromtxt(fname, dtype, comments, delimiter, skip_header, skip_footer, converters, missing_values, filling_values, usecols, names, excludelist, deletechars, replace_space, autostrip, case_sensitive, defaultfmt, unpack, usemask, loose, invalid_raise, max_rows, encoding, ndmin, like)
...
[^1]: 使用 genfromtxt 导入数据—NumPy v1.26 手册—NumPy 中文. (n.d.). From https://numpy.com.cn/doc/stable/user/basics.io.genfromtxt.html
[^2]: NumPy IO | 菜鸟教程. (n.d.). From https://www.runoob.com/language/python/numpy/numpy-io.html
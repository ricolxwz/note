...
详情见[这里](/language/python/foundation/container/#列表生成式).

### 匿名函数

匿名函数即Lambda函数, 详情见[这里](/language/python/foundation/function/#Lambdafunction).

### `map`函数

`map`函数, 详情见[这里](/language/python/foundation/function/#mapfunction).
...
详情见[这里](/language/python/numpy/array-creation).

### 数组索引

详情见[这里](/language/python/numpy/array-index).

### 广播

详情见[这里](/language/python/numpy/broadcast).

### 数组变形与合并

#### 转置

访问数组的`T`属性, 返回的数组是一个[视图](/language/python/numpy/view-copy/#视图).
...
`np.nonzero`函数返回的时非零数的索引. 这个函数可用于[布尔数组索引](/language/python/numpy/array-index/#布尔array-index)到整数数组索引的转换.
...
[^1]: 第一章 预备知识—Joyful Pandas 1.0 documentation. (n.d.). From https://inter.joyfulpandas.datawhale.club/Content/ch1.html
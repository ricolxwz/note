...
元组可以表示不变的集合, 为了表示的清晰, 我们可以基于元组类`tuple`创建自己的类, `collections`模块中的`namedtumple`函数内部通过调用[`type()`函数](/language/python/foundation/object/#type)可以创造一个`tuple`的子类, 允许我们自定义这个类的名称, 所以被称为命名元组.
...
- `[dequue_instance].extend([iterable])`: 在右端扩展[可迭代对象](/language/python/foundation/container/#迭代器和可迭代对象的区别)中的所有元素
- `[dequue_instance].extendleft([iterable])`: 在左端扩展[可迭代对象](/language/python/foundation/container/#迭代器和可迭代对象的区别)中的所有元素
...
???+ warning "注意"
    ...
    默认值时调用函数返回的, 函数在创建`defaultdict`对象时作为[高阶函数](/language/python/foundation/function/#高阶function)传入.
...
???+ example "例子"
    ...
    什么时候用链式字典最合适? 应用程序往往需要传入参数, 参数可以通过[命令行传入](/language/python/basic-package/argparse), 可以通过环境变量传入, 还可以有默认参数, 我们可以用ChainMap实现参数的优先级查找, 即先查命令行参数, 如果没有传入, 再查环境变量, 如果没有, 就是用默认参数.
...
???+ example "例子"
   ...
   [^1]: Collections. (n.d.). From https://www.liaoxuefeng.com/wiki/1016959663602400/1017681679479008
...
1. 文本模式
    ...
    - `r`: 读取模式, 文件用于读取, 如果文件不存在, 会引发`FileNotFoundError`, 自动[解码](/language/python/foundation/encoding)
    - `w`: 写入模式, 如果文件存在会被清空, 如果文件不存在则创建新的文件, 自动[编码](/language/python/foundation/encoding)
    - `a`: 追加模式, 如果文件存在, 写入内容追加到文件末尾, 如果文件不存在则穿件新的文件, 自动[编码](/language/python/foundation/encoding)
    - `x`: 独占创建模式: 创建新文件用于写入, 如果文件已经存在则引发`FileExistsError`, 自动[编码](/language/python/foundation/encoding)

2. 二进制模式

    - `rb`: 读取二进制文件, 没有[解码](/language/python/foundation/encoding)的过程 
    - `wb`: 写入二进制文件, 没有[编码](/language/python/foundation/encoding)的过程 
    - `ab`: 追加二进制文件, 没有[编码](/language/python/foundation/encoding)的过程 
    - `xb`: 独占创建二进制文件, 没有[编码](/language/python/foundation/encoding)的过程 

3. 文本模式附加

    - `r+`: 读取和写入模式, 文件必须存在, 自动[解码](/language/python/foundation/encoding)
    - `w+`: 写入和读取模式, 会清空文件内容, 如果文件不存在则创建新文件, 自动[编码](/language/python/foundation/encoding)
    - `a+`: 追加和读取模式, 如果文件不存在则创建新文件, 自动[编码](/language/python/foundation/encoding)
    - `x+`: 独占创建和读取模式, 如果文件已存在则引发`FileExistError`, 自动[编码](/language/python/foundation/encoding)

4. 组合模式

    - `rb+`或`r+b`：读取和写入二进制文件, 没有[解码](/language/python/foundation/encoding)的过程
    - `wb+`或`w+b`：写入和读取二进制文件, 没有[编码](/language/python/foundation/encoding)的过程
    - `ab+`或`a+b`：追加和读取二进制文件, 没有[编码](/language/python/foundation/encoding)的过程
    - `xb+`或`x+b`：独占创建和读取二进制文件, 没有[编码](/language/python/foundation/encoding)的过程
...
???+ example "例子"
    ...
    'posix'
    >>> os.uname()
    posix.uname_result(sysname='Darwin', nodename='wenzexus-Mac-mini.local', release='23.5.0', version='Darwin Kernel Version 23.5.0: Wed May  1 20:19:05 PDT 2022; root:xnu-10063.121.3~5/RELEASE_ARM64_T8112', machine='arm64')
    ```
...
???+ example "例子"
    ...
    === "序列化"
        ...
        ???+ tip "Tip"
            ...
            有一种更加简便的方法定义转化函数, 那就是用实例的`__dict__`属性, 它就是一个字典, 用来存储实例变量, 所以我们只需要将`default`参数设置为一个返回对象`__dict__`属性的[Lambda函数](/language/python/foundation/function/#Lambdafunction)就可以了.
...
[^1]: 文件读写. (n.d.). From https://www.liaoxuefeng.com/wiki/1016959663602400/1017607179232640
[^2]: 操作文件和目录—廖雪峰的官方网站. (n.d.). From https://www.liaoxuefeng.com/wiki/1016959663602400/1017623135437088
[^3]: 序列化. (n.d.). From https://www.liaoxuefeng.com/wiki/1016959663602400/1017624706151424
[^4]: StringIO和BytesIO - 廖雪峰的官方网站. (n.d.). From https://www.liaoxuefeng.com/wiki/1016959663602400/1017609424203904
...
1. 转发表的设置🌟: 我们是如何联合运用inter-AS算法和intra-AS算法设置转发表的呢? 举个例子, AS1通过inter-AS算法学到了子网x只能通过AS3, __网关1c到达__, inter-AS协议将这个信息传播给AS1内的所有路由器, 路由器1d使用intra-AS算法确定接口I是到达网关1c的最短路径, 在转发表中设置条目(x, I).
2. 多个AS选择: AS1学到子网x既可以通过AS3也可以通过AS2到达, 路由器1d决定转发到哪个网关, 这也是inter-AS路由协议的工作. 通常在这种情况下, 使用的是热土豆路由, 也就是选择离自己最近的网关转发数据包, 这样可以减少本AS内的流量负担.

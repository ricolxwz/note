...
首先, 我们要消除冗余约束, 子集$S$的边界和补集的边界是一样的(同一组边), $2^n-2$个约束实际上只有$(2^n-2)/2$个独立约束. 所以, 我们可以得到$\sum_{e\in \delta(S)}x_e = \sum_{e\in \delta(V/\ S)}x_e$.

其次, 子环消除约束$\sum_{e\in \delta(S)}x_e\geq 2$由于

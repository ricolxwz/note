...
关系模型(Relational Model)是一种逻辑模型(Logical Model), 逻辑模型是什么详见[这里](/general/database/conceptual-model/#两类conceptual-model).
...
???+ example "例子"
    ...
    得到的表格如[图](http://img.ricolxwz.asia/e7bd17ea2a47fbe0c29af21a62050751.png).
...
???+ example "例子"
    ...
    === "例子1"
        ...
        [见图](http://img.ricolxwz.asia/97077b1a1099dfb5cf9b58a468008ba6.png)
...
对于引用的关系中每个外键值为$\alpha$的元组, 一定要在被引用的关系中存在一个候选键也为$\alpha$的元组, 如[图](http://img.ricolxwz.asia/3578df3777540c7e36266edb3b958db2.png).
...
没有约束的关系即对于实体集和某一个关系来说, 实体集没有键约束, 参与约束, 基数约束. 想要映射这种关系特别简单, 只要将实体集的主键的组合作为新的关系的主键就行了. 如[图](http://img.ricolxwz.asia/f7a07b8706af6f4dab96d5a946ee93a2.png).
...
可以将参与关系的实体集的关系和关系集的关系组合变成一个新的关系. 太复杂了....直接看[图](http://img.ricolxwz.asia/c8203c4430ba88d59b0ccb708c07a162.png).

若要表示两侧都有键约束, 可以在外键上施加一个唯一性限制. 直接看[图](http://img.ricolxwz.asia/d528c111b390896090bb774e9fd92fab.png).
...
我们以两侧都有键约束为例, 现在在一侧加入一个参与约束, 要实现将这个参与约束映射到关系中, 就要使得这个外键不为空, 即为`NOT NULL`. 根据上面讲到的, 要实现两侧的键约束, 外键还需要是唯一的, 所以外键: 1. 既是唯一的; 2. 又是非空的. 见[图](http://img.ricolxwz.asia/fca7486e063b1bc75f1ef25d31873e46.png).

### 映射弱实体型 {#map-weakstrongentity}

什么是弱实体型, 见[这里](/general/database/conceptual-model/#强弱实体型).

弱实体型映射到一个主键是区分符和强实体型的主键的组合的关系中. 如[图](http://img.ricolxwz.asia/ec2b4323b6856140eea624640c405c24.png).
...
映射isA结构的时候, 子类的外键是超类(高层次的, 抽象的类)的主键. 如[图](http://img.ricolxwz.asia/e24aee851049a321ea87670a3368daf6.png)

### 映射聚合 {#map-aggregation}

聚合之后的新的实体型的键和另一个参与聚合关系的实体的键构成了聚合关系的键和外键. 如[图](http://img.ricolxwz.asia/e5f435a531088490c3c6c0eb1ce55426.png).

...
可以简单的理解为外键不能出现悬空引用, 如[图](http://img.ricolxwz.cn/e0fa20affecb5a6d2c2498656ef05f81.png). 理解外键不能出现悬空之后, 就可以很好的理解外键约束在父表上执行删除或更新操作时的几种选项:
...
???+ tip "Tip"
    ...
    - SQL不支持"对于所有的X, 条件P(X)成立"的语法, 而且这非常浪费时间. 我们可以对这个式子取反命题, 即"对于所有的X, 条件P(X)成立"等价于"不存在一个X, 使得P(X)不成立", 这个反命题的写法SQL是支持的, 首先, 写一个`SELECT`语句选出所有的违反条件P(X)的元组, 然后使用`NOT EXISTS`语句, 如果返回元组的个数为0, 说明原式成立, 反之, 不成立
        ...
        ???+ example "例子"
            ...
            - 如[图](http://img.ricolxwz.cn/be08a9f2e269d73038f5e78ec3028264.png), 这个操作是被拒绝的
            - 如[图](http://img.ricolxwz.cn/a043d3639a5fd6999bdb4e11235270a3.png), 这个操作是被允许的
...
两者之间的详细比较可以见[图](http://img.ricolxwz.cn/afff0b0693df1a0fb6ef9a9817294292.png).
...
如[图](http://img.ricolxwz.cn/090a3963ffb6015e442186321013f041.png).
...
如[图](http://img.ricolxwz.cn/e54f8653a17c64b34c7828998f459fb5.png).

#### 语法

见[图](http://img.ricolxwz.cn/8ea78f6df2632bc7afba00011b8240ba.png).
